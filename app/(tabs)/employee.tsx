      ...
      {employees.length > 0 ? (
        <View>
          {employees
            .filter(
              (employee) =>
                employee?.userId?.fullName.includes(input.toLowerCase()) // Filter based on search input
            )
            .map((employee) => (
              <Pressable
                key={employee?.userId?._id}
                onPress={() =>
                  router.push(`/employee/${employee?.userId?._id}`)
                } // Navigate to employee details page on click
                style={{
                  padding: 10,
                  borderBottomWidth: 1,
                  borderColor: "#E0E0E0",
